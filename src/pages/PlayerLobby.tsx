...
const PlayerLobby = () => {
  ...
  config: {
    ...
    clanAssignment: data.clan_assignment,
    clanNames: {
...
setQuiz(normalizedQuiz);
  ...
  const fetchPlayers = async () => {
   ...
   // setAllPlayers(data || []);
   const normalizedPlayers: Player[] = (data || []).map(p => ({
  id: p.player_id,
  name: p.player_name,
  avatar: p.avatar,
  clan: p.clan,
...
const normalizedQuiz: Quiz = {
  quizId: q.quiz_id,
  gameState: q.game_state,
  config: {
    clanBased: q.clan_based,
    clanAssignment: q.clan_assignment,
    clanNames: {
      [Clan.TITANS]: q.titan_name || 'Titans',
      [Clan.DEFENDERS]: q.defender_name || 'Defenders',
    },
  },
};
...
if (normalizedQuiz.gameState !== GameState.LOBBY) {
  navigate(`/quiz/player/${quizId}/${playerId}`);
}


        if (updatedQuiz.game_state !== GameState.LOBBY) {
          navigate(`/quiz/player/${quizId}/${playerId}`);
        }
      }
    )
