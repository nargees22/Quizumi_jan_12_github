...
// //import { generateQuestions } from '../service/gemini';
...
//import { db } from '../firebase.ts';useEffec
...
const CreateQuizPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchLibraryData = async () => {
            ...
            try {
                ...
                const mappedQuestions: Question[] = (data || []).map((row: any) => ({
                    id: row.pk_id.toString(),          // ✅ REQUIRED for React keys
                    db_pk_id: row.pk_id,               // optional, but useful
                    text: row.question_text,           // ✅ UI uses q.text
                ...
                if (mappedQuestions.length > 0) {
                    ...
                    const techList = Array.from(techMap.values()).sort((a, b) => a.localeCompare(b));
                    const skillList = Array.from(skillMap.values()).sort((a, b) => a.localeCompare(b));

                    setTechnologies(techList);
                    setMasterSkills(skillList);
                    setSkillsForFilter(skillList);
                }
        ...
        const fetchQuizzes = async () => {
            ...
            try {
                ...
                const quizzesData = data as Quiz[];
                ...
                const drafts = quizzesData
                    .filter(q => q.is_draft === true)
                    .sort(
                        (a, b) =>
    ...
    const filteredQuestions = useMemo(() => {
        const sourceQuestions =
            libraryView === 'mine'
                ? libraryQuestions.filter(q => q.organizer_name === organizerName)
                : libraryQuestions;
    ...
    const handleAddToLibrary = async () => {
        ...
        try {
            ...
            if (customQuestion.type === QuestionType.MCQ) {
                questionData.option_1 = customQuestion.options[0] ?? null;
                questionData.option_2 = customQuestion.options[1] ?? null;
                questionData.option_3 = customQuestion.options[2] ?? null;
                questionData.option_4 = customQuestion.options[3] ?? null;
                questionData.correct_answer_index = customQuestion.correctAnswerIndex;
            }
            ...
            const { data, error } = await supabase
                ...
                // .insert([{ ...questionData, creationTime: new Date().toISOString() }])
                .insert([questionData])
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            await supabase
                .from('quiz_master_structure')
                // .insert([{ ...quiz, startTime: new Date().toISOString() }]);
                .insert([{
    ...
    const handleStartLiveQuiz = async () => {
        ...
        const questionRows = questions.map((q, index) => ({
            ...
            correct_answer_index:
                q.type === QuestionType.MCQ ? q.correctAnswerIndex ?? 0 : null,
        }));
        ...
        /* ------------------ 3️⃣ NAVIGATE SAFELY ------------------ */
        localStorage.setItem(`quiz-host-${quizId}`, hostId);
    ...
    //                     end_time: null,
    //                 })
    //                 .eq('quiz_id', editingDraft.quiz_id);

    //             localStorage.setItem(`quiz-host-${editingDraft.id}`, hostId);
    //             navigate(`/lobby/${editingDraft.quiz_id}`);
    //         } catch (error) {
    ...
    const handleGenerateQuestions = async () => {
        ...
        try {
            ...
            //     .upsert({
            //         organizerName,
            //         [today]: aiUsage + validQuestions.length,
            //     });
    ...
    const handleAddFromGenerator = async (questionDataToAdd: Omit<Question, 'id'> & { status?: 'adding' | 'added' }, index: number) => {
        ...
        try {
            ...
            const { data, error } = await supabase
                ...
                //.insert([{ ...questionData, creationTime: new Date().toISOString() }])
                .insert([{
                    question_text: questionCoreData.text,
                    time_limit: questionCoreData.timeLimit ?? 30,
                    technology: questionCoreData.technology?.toLowerCase() ?? '',
                    skill: questionCoreData.skill?.toLowerCase() ?? '',
                    type: questionCoreData.type,
                    organizer_name: organizerName,
                    creation_time: new Date().toISOString(),

                    option_1: questionCoreData.options?.[0] ?? null,
                    option_2: questionCoreData.options?.[1] ?? null,
                    option_3: questionCoreData.options?.[2] ?? null,
                    option_4: questionCoreData.options?.[3] ?? null,

                    correct_answer_index: questionCoreData.correctAnswerIndex ?? null,
                }])
                .select()
                .single();
            ...
            // const newLibraryQuestion = data as Question;
            ...
            // );
            // ✅ Map DB row → UI Question shape (IMPORTANT)
            ...
            // 1️⃣ Add to main source (TOP of list)
            setLibraryQuestions(prev => [newQuestion, ...prev]);

            // 2️⃣ Ensure technology filter includes it
            setTechnologies(prev =>
                prev.includes(newQuestion.technology)
                    ? prev
                    : [...prev, newQuestion.technology]
            );

            // 3️⃣ Ensure master skills list includes it
            setMasterSkills(prev =>
                prev.includes(newQuestion.skill)
                    ? prev
                    : [...prev, newQuestion.skill]
            );

            // 4️⃣ VERY IMPORTANT: update skillsForFilter
            setSkillsForFilter(prev =>
                prev.includes(newQuestion.skill)
                    ? prev
                    : [...prev, newQuestion.skill]
            );

            // 5️⃣ Mark AI question as added
            setGeneratedQuestions(prev =>
                prev.map((q, i) => (i === index ? { ...q, status: 'added' } : q))
            );
                                                ...
                                                filteredDrafts.map(q => (
                                                    <div key={q.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-50 p-3 rounded-lg gap-2">
                                                        <div>
                                                            ...
                                                            <p className="text-sm text-slate-500">{(q.questions?.length ?? 0)
                                                            } Questions</p>
                                                        </div>
                                                                        ...
                                                                        <div key={q.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-50 p-3 rounded-lg gap-2">
                                                                            <div>
                                                                                ...
                                                                                <div className="mt-2 flex items-baseline gap-x-4 gap-y-1 flex-wrap">
                                                                                    <div className="flex items-center gap-1.5 text-slate-600"><span className="font-bold text-slate-800 text-base">{(q.questions?.length ?? 0)}</span><span className="text-sm">Questions</span></div>
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <div className="mt-2 flex items-baseline gap-x-4 gap-y-1 flex-wrap">
                                                                            <div className="flex items-center gap-1.5 text-slate-600">
                                                                                <span className="font-bold text-slate-800 text-base">{(q.questions?.length ?? 0)
                                                                                }</span>
