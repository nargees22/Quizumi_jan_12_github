...
const QuizPlayerPage = () => {
        ...
        const playersChannel = supabase
            ...
            .on(
                ...
                async () => {
                    ...
                    setAllPlayers(data || []);
                    setPlayer(data?.find(p => p.player_id === playerId) || null);
                }
            )
            .subscribe();
    ...
    const submitAnswer = useCallback(async (answerPayload: PlayerAnswer['answer']) => {
        if (!quiz || !player || !question || !quizId || !playerId) return;

        setIsAnswerLocked(true);
        playSound('survey');

        const timeTaken =
            (Date.now() - (localQuestionStartTimeRef.current || Date.now())) / 1000;
    ...
    }, [quiz, player, question, quizId, playerId]);
        ...
        try {
            ...
            if (type === 'fiftyFifty') {
                await supabase
                    .from('quiz_players')
                    .update({
                        score: player.score - fiftyFiftyCost,
                        fiftyFiftyUses: (player.fiftyFiftyUses || 0) + 1,
                    })
                    .eq('player_id', playerId);
            }
    ...
    const renderContent = () => {
        ...
        if (quiz.gameState === GameState.LEADERBOARD) {
            return (
                <div className="flex-grow w-full flex flex-col items-center justify-center p-4 sm:p-8">
